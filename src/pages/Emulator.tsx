...
if (temp === null) {
  ...
  // 기존 localStorage 값이 있을 경우 해당 gpx 데이터를 사용
  const index = gpxNameList.indexOf(temp)
  if (index !== -1) {
    allGpx = gpxList[index]
  } else {
    // fallback 발생 시: 기본값 gpx01 + 잘못된 localStorage 정정
    allGpx = gpx01
...
export default function Emulator() {
  ...
  useEffect(() => {
    if (!userId) return
    axios
      .get(BASE_URL + '/api/v1/reservation/allocation')
      .then(res => {
        setVehicleId(res.data.data.vehicleId)
        setDueDate(res.data.data.dueDate)
        setReservationStatus(res.data.data.status)
        console.log(res)
      })
      .catch(() => {
        setVehicleId(null)
  ...
  useEffect(() => {
    ...
    if (engineOn && vehicleId) {
      interval = setInterval(() => {
        // 현재 구간 추출
        const cycle_infos = allGpx.slice(
          cycleStartIdx,
          cycleStartIdx + CYCLE_SIZE
        )
